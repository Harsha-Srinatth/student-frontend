...
const CreateClubAnnouncement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch clubs
        const clubsResponse = await api.get("/faculty/clubs/my");
        ...
        if (isEditMode) {
          ...
          if (announcement) {
            ...
            try {
              const annResponse = await api.get(`/faculty/clubs/announcements/${id}`);
              ...
              setTimeout(() => navigate("/faculty/announcements"), 2000);
            }
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (isEditMode) {
        response = await api.put(`/faculty/clubs/announcements/${id}`, postData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
      } else {
        response = await api.post("/faculty/clubs/announcements", postData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
